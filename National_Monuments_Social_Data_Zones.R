...
econ_sf <- st_read(paste(infolder,"econ_shp.shp", sep=""), stringsAsFactors=FALSE)
econ_sf_proj <- st_transform(econ_sf, st_crs(PA))
...
var <- c("Farm", "Forestry", "Mine")
val_name <- c("max_Frm", "mx_FrNR", "mx_Mnng")
...
for (i in 1:length(buffer_dist)){
  ...
  temp.int.df <- st_intersection(econ_sf_proj, PA.buf)  # intersect PA buffers and county/congressional district polygons
...
write.csv(soc.data.df, paste0(outfolder, "econData_12_7_17.csv"), row.names=FALSE)


LCV_sf <- st_read(paste(infolder,"LCV_shp.shp", sep=""), stringsAsFactors=FALSE)
LCV_sf_proj <- st_transform(LCV_sf, st_crs(PA))
...
write.csv(soc.data.df, paste0(outfolder, "LCVData_12_7_17.csv"), row.names=FALSE)
