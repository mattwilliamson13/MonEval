...
for (i in 1:length(buffer_dist)){
  PA.buf <- st_buffer(PA, dist = buffer_dist[i]) %>%  # buffer by specified distance
    mutate(buffered_area_m2 = as.numeric(st_area(geometry)))  # calculate area of each buffer polygon
  temp.int.df <- st_intersection(ind_will_sf, PA.buf)  # intersect PAs and county/congressional district polygons
...
write.csv(soc.data.df, paste0(outfolder, "socData_10_20_17.csv"), row.names=FALSE)
